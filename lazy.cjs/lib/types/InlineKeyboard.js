...
class InlineKeyboard {
  ...
  setRows(rows) {
    this.rows = rows ? InlineKeyboard.normalize(rows).map(row => row.map(button =>
      button instanceof types.InlineKeyboardButton ? button : new types.InlineKeyboardButton(button.text, button, this.context)
    )) : []
    return this
  }
...
InlineKeyboard.fromParams = (params = [], context) => new InlineKeyboard(params.map(row =>
  row.map(button => types.InlineKeyboardButton.fromParams(button, context))
), context)
