...
class Chat {
  ...
  get isPrivate() {
    return this.type === constants.ChatType.PRIVATE
  }

  get isGroup() {
    return this.type === constants.ChatType.GROUP
  }

  get isSupergroup() {
    return this.type === constants.ChatType.SUPERGROUP
  }

  get isChannel() {
    return this.type === constants.ChatType.CHANNEL
  }
  ...
  setPhoto(photo) {
    this.photo = photo ? (
      photo instanceof types.ChatPhoto ? photo : new types.ChatPhoto(photo, {
        ...this.context,
        chatId: this.id
      })
    ) : undefined
  ...
  setLocation(location) {
    this.location = location ? (
      location instanceof types.ChatLocation ? location : new types.ChatLocation(location, this.context)
    ) : undefined
    return this
  }
  ...
  sendAction(action) {
    return this.context.telegramBot.sendconstants.ChatAction(this.id, action)
  }

  sendActionTyping() {
    return this.context.telegramBot.sendconstants.ChatActionTyping(this.id)
  }

  sendActionUploadPhoto() {
    return this.context.telegramBot.sendconstants.ChatActionUploadPhoto(this.id)
  }

  sendActionRecordVideo() {
    return this.context.telegramBot.sendconstants.ChatActionRecordVideo(this.id)
  }

  sendActionUploadVideo() {
    return this.context.telegramBot.sendconstants.ChatActionUploadVideo(this.id)
  }

  sendActionRecordAudio() {
    return this.context.telegramBot.sendconstants.ChatActionRecordAudio(this.id)
  }

  sendActionUploadAudio() {
    return this.context.telegramBot.sendconstants.ChatActionUploadAudio(this.id)
  }

  sendActionUploadDocument() {
    return this.context.telegramBot.sendconstants.ChatActionUploadDocument(this.id)
  }

  sendActionFindLocation() {
    return this.context.telegramBot.sendconstants.ChatActionFindLocation(this.id)
  }

  sendActionRecordVideoNote() {
    return this.context.telegramBot.sendconstants.ChatActionRecordVideoNote(this.id)
  }

  sendActionUploadVideoNote() {
    return this.context.telegramBot.sendconstants.ChatActionUploadVideoNote(this.id)
  }
...
Chat.fromParams = (params = {}, context) => {
  ...
  if (data.photo) {
    data.photo = types.ChatPhoto.fromParams(data.photo, {
      ...context,
      chatId: data.id
    })
  }
  if (data.pinnedMessage) {
    data.pinnedMessage = types.Message.fromParams(data.pinnedMessage, context)
  }
  if (data.permissions) {
    data.permissions = types.ChatPermissions.fromParams(data.permissions, {
      ...context,
      chatId: data.id
    })
  }
  if (data.location) {
    data.location = types.ChatLocation.fromParams(data.location, {
      ...context,
      chatId: data.id
    })
  }
