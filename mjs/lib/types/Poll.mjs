...
import { PollType } from '../constants/index.mjs'
import PollOption from './PollOption.mjs'
...
export default class Poll {
  ...
  static fromParams(params = {}, context) {
    ...
    if (data.options) {
      data.options = data.options.map(option => PollOption.fromParams(option))
    }
    if (data.explanationEntities) {
      data.explanationEntities = data.explanationEntities.map(entity => MessageEntity.fromParams(entity, {
        ...context,
  ...
  setOptions(options) {
    this.options = options ? options.map(option =>
      option instanceof PollOption ? option : new PollOption(option)
    ) : undefined
    return this
  }
